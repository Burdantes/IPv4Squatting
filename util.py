...
def load_unannounced_default():
    default_file = os.path.join(this_dir, 'data/updated-unannounced-prefix/unannounced-202009.txt')
    # default_file = os.path.join(this_dir,'../data/CGNAT.txt')

    return load_squatspace_default(default_file)



def load_squatspace_default(announced = 'unannounced-202009.txt'):
    # if announced != 'unannounced-202009.txt':
        announced = 'data/updated-unannounced-prefix/' + announced
        # announced = 'data/' + announced
        default_file = os.path.join(this_dir, announced)
        trie = radix.Radix()
        dico = {}
        ...
        #     print(rnode.prefix)
        # liste = []
        # for t in tqdm(trie.prefixes()):
        #     prefix = t
        #     l = [str(ip) for ip in ipaddress.IPv4Network(prefix)]
        #     if len(l) > 10**8:
        #         print(prefix)
        #     liste.extend(l)
        ...
        # liste = list(set(liste))
        # import csv
        # with open(announced.split('.')[0] + '.csv', 'w', newline='') as myfile:
        #     wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
        #     wr.writerow(liste)
        # dg = pd.DataFrame()
        # dg['Squat_IP'] = pd.Series(list)
        # # dg['Squat_IP'] = dg
        # print(dg)
        # dg.to_csv(announced.split('.')[0] + '.csv')
        return trie
...
def load_asn2org_default():
    default_file = os.path.join(this_dir, 'data/BGP/20210401.as-org2info.txt')
    return load_asn2org(default_file)

def load_dod_prefix():
    list_of_prefixes_dod = ['6.0.0.0/8', '7.0.0.0/8', '11.0.0.0/8', '21.0.0.0/8', '22.0.0.0/8', '26.0.0.0/8', '28.0.0.0/8', '29.0.0.0/8','30.0.0.0/8', '33.0.0.0/8', '55.0.0.0/8', '214.0.0.0/8', '215.0.0.0/8']
    return list_of_prefixes_dod

def load_squat_prefix():
    list_of_prefixes_squat = ['56.0.0.0/8', '48.0.0.0/8', '43.0.0.0/8',
...
def load_ip2asn_default(bdrmapIT=False):
    default_ixprefixes_file = os.path.join(this_dir, "data/BGP/ix_prefixes_20210507.txt")
    if bdrmapIT:
        return load_ip2asn_bdrmapIT(default_ixprefixes_file)
    else:
        default_ipasn_file = os.path.join(this_dir, "data/BGP/prefix-asn-202105.txt.gz")
    default_ixprefixes_file = os.path.join(this_dir, "data/BGP/ix_prefixes_20210507.txt")
    return load_ip2asn(default_ipasn_file, default_ixprefixes_file)


def load_ip2asn_bdrmapIT(ip_ixp_filename):
    print('HERE')
    with open(
            '/Users/geode/Documents/Datasets/InternetMeasurements/Arkipelago/ITDK/ITDK-2020-08/bdrmapIT_output.json') as f:
        data = json.load(f)
    ...
    with open(ip_ixp_filename) as f:
        for line in f:
            line = line.strip()
            ixp, prefix = line.split('\t')
            pyt.insert(prefix, ixp)
            # node.data["asn"] = ixp
    return pyt


def comparing_mappings():
    trie2 = load_ip2asn_default(True)
    trie1 = load_ip2asn_default()
    count_same = 0
    count_different = 0
    for ip in tqdm(trie2):
        node = trie1.search_best(ip)
        if node is None:
            continue
        if trie2[ip] != node.data['asn']:
            count_different += 1
            # print(ip,trie2[ip],node.data['asn'])
        else:
            count_same += 1
            # print('Success')
    print('THIS IS THE KEY OUTPUT', count_same, count_different)
    print(float(count_same) / (count_same + count_different))
