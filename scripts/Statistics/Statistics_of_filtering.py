...
def computing_empirical_frequencies(dg,min_traceroute=5000,min_source = 50):
    ...
    plt.savefig('../results/fract_squat.pdf')
...
def generating_stats(df,ms = [50,100,500,1000,2500,5000,10000,10**5,10**6,10**7],ns = [2,5,10,25,50,100,250,500,1000,2500,5000]):
    ...
    for m in ms:
        ...
        frequency = 0
        frequency_z = 0
        for t in df.groupby('endUserASN'):
            to_check.append(t[0] +'\t')
            val1 = float(t[1][t[1].index==t[1].index[0]]['#Field:count'].values[0])
            ind1 = t[1][t[1].index==t[1].index[0]]['#Field:count'].index.values[0]
            # res_stat[ind1] = val1
            ...
            p = res[t[0]]
            bin = 1 - stat.binom.cdf(k=1,n=m,p=p)
            # print(bin)
            if bin >random():
        ...
        ser[m] = [m,float(frequency_z)/frequency,'# of traceroutes']
        print(m,succ)
        # res_stat[ind2] = val1
    # print(res)
    ...
    for n,m in zip(ns,ms):
        ...
        for t in df.groupby('endUserASN'):
            ...
            res[t[0]] = float(val2)/float(val1+val2)
            p = res[t[0]]
            bin = 1 - stat.binom.cdf(k=1,n=n,p=p)
            # print(bin)
            if bin > random():
            # if float(m)/val1 < res[t[0]]:
                res_stat[t[0]] = 'Success'
        ...
        succ = dict(Counter(list(res_stat.values())))
        # for t in succ:
        ser[n] = [n,frequency_z/float(frequency),'# of source IP']
    ...
    for n in ns:
        ...
        succ = dict(Counter(list(res_stat.values())))
        # for t in succ:
        ser[n] = [n,float(frequency_z)/float(frequency),'# of source IP']
...
def visualisation(dg,dl,dh):
    ...
    ax.set_xlabel(r"# of Traceroutes")
    ax2.set_xlabel(r"# of Source IP")
    ax.set_ylabel(r"$P_\text{miss}$")
    # ax2.set_ylabel(r"Temperature ($^\circ$C)")
    ax2.set_xlim(10 ** -1, 10 * 10 ** 3)
    ax.set_xlim(10, 10 ** 6)
    ax.set_ylim(0,0.1)
    plt.savefig('../../statistics_selection_update.pdf')

if __name__ == "__main__":
    #
    ms = [200, 500, 1000, 2500, 5000, 100000, 250 * 10 ** 2, 1000*100, 5 * 10 ** 5]
    ns = [2, 5, 10, 25, 50, 100, 250, 1000, 5000]
    df = reading_data()
